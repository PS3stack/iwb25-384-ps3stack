...
service /auth on new http:Listener(HTTP_PORT) {
    ...
    resource function post voter/login(auth:LoginRequest req) returns auth:LoginResponse|http:Response|error {
        auth:LoginResponse|error response = auth:loginUser(req);
        if response is error {
            return error("Error logging in voter: " + response.message());
        }
        if response.success && response.token is string {
            http:Response httpResponse = new;
            http:Cookie authCookie = auth:createAuthCookie(response.token.toString(), 1);
    ...
    resource function post createSampleData() returns json|error {
        // First, create the voter role if it doesn't exist
        var roleResult = auth:insertRole(5, "voter");
        if roleResult is error {
            io:println("Failed to insert voter role: " + roleResult.message());
        } else {
            io:println("Successfully inserted/verified voter role");
        }
        ...
        auth:User[] sampleUsers = [
            ...
            {name: "John Voter", email: "voter1@test.com", password_hash: "password123", role_id: 5},
            {name: "Alice Voter", email: "voter2@test.com", password_hash: "password123", role_id: 5},
            {name: "Bob Voter", email: "voter3@test.com", password_hash: "password123", role_id: 5},
            {name: "Carol Voter", email: "carol.voter@test.com", password_hash: "password123", role_id: 5},
            {name: "David Voter", email: "david.voter@test.com", password_hash: "password123", role_id: 5}
        ];
